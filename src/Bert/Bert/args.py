...
def setup_train_args():
    ...
    parser.add_argument('--train_raw_path', default='data/train_Middle.txt', type=str, required=False, help='原始训练语料')
    parser.add_argument('--train_tokenized_path', default='data/train_tokenized.txt', type=str,
                        required=False,
                        help='将原始训练语料tokenize之后的数据的存放位置')
    parser.add_argument('--valid_raw_path', default='data/valid_Middle.txt', type=str, required=False, help='原始验证语料')
    parser.add_argument('--valid_tokenized_path', default='data/valid_tokenized.txt', type=str,
                        required=False,
                        help='将原始验证语料tokenize之后的数据的存放位置')
    parser.add_argument('--test_raw_path', default='data/test_Middle.txt', type=str, required=False, help='原始测试语料')
    ...
    parser.add_argument('--valid_mmi_tokenized_path', default='data/valid_mmi_tokenized.txt', type=str,
                        required=False,
                        help='将原始验证语料的每段对话翻转，然后进行tokenize之后的数据的存放位置，用于训练MMI模型')
    parser.add_argument('--test_mmi_tokenized_path', default='data/test_mmi_tokenized.txt', type=str,
                        required=False,
...
def train(model, device, train_list, multi_gpu, args, valid_list, test_list):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, input_ids in enumerate(train_dataloader):
            ...
            input_ids = input_ids.to(device)
            # print('ip',input_ids.size())
            # 解决在运行过程中，由于显存不足产生的cuda out of memory的问题
            try:
                outputs = model.forward(input_ids=input_ids)
                # print('out', outputs)
                # print('losss',outputs.loss)
...
def main():
    ...
    elif args.raw and not args.train_mmi:
        print('preprocessing')  # 如果当前是要训练对话生成模型
